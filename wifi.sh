...
#
# Note: wpa_passphrase <ssid> <ssid-password> generates the 'ConfigFile'
#
...
ConfigFile="/etc/wpa_supplicant/default.conf"
...
do_start()
{
  ...
  # Checking if default config file exists
  test -f "$ConfigFile"
  assert "$? -eq 0" "Config file '$ConfigFile' missing"

  # launch it
  $Wsupplicant -i $Ifname -c "$ConfigFile" -B
  if [ $? -ne 0 ]; then
